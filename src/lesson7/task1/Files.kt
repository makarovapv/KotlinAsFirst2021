...
fun deleteMarked(inputName: String, outputName: String) {
    File(outputName).bufferedWriter().use {
        for (line in File(inputName).readLines()) {
            if (line.isEmpty()) it.newLine() else {
                if (line[0] == '_') continue else {
                    it.write(line)
                    it.newLine()
                }
            }
...
fun chooseLongestChaoticWord(inputName: String, outputName: String) {
    ...
    val listForRemove = mutableListOf<String>()
    File(outputName).bufferedWriter().use { it ->
        var maxCount = 0
        val countList = mutableListOf<Int>()
        File(inputName).readLines().joinToString(" ").split(" ").forEach { word ->
            val newWord = word.lowercase()
            if (newWord.length >= maxCount && newWord.length == newWord.split("").toSet().filter { it !in "" }.size) {
                maxCount = newWord.length
                countList.add(newWord.length)
                result.add(word)
            }
        }
        countList.forEachIndexed { index, value ->
            if (value < maxCount) {
                listForRemove.add(result[index])
            }
        }
...
fun markdownToHtmlSimple(inputName: String, outputName: String) {
    ...
    File(outputName).bufferedWriter().use {
        ...
        var p = false
        if (text.replace("__NEWLINE__", "\n").isBlank()) it.write("<p></p>")
